...
const AdminView = {
        ...
        return `
            <section class="section">
                <div class="page-header">
                    ...
                    <h1 class="section__title">🔐 Moderátorský panel</h1>
                    <p class="section__description">Zadejte heslo pro přístup k moderátorským funkcím.</p>
                </div>
                <form class="form admin-login-form" onsubmit="AdminView.handleLogin(event)">
                    <div class="form__group">
                        <label class="form__label" for="admin-password">Heslo</label>
                        <input class="form__input" type="password" id="admin-password" 
                            placeholder="Zadejte heslo moderátora" required>
                    </div>
            ...
            <section class="section">
                <div class="page-header">
                    ...
                    <div class="admin-header">
                        <h1 class="section__title">🛠️ Moderátorský panel</h1>
                        <button class="button button--secondary button--small" onclick="AdminView.logout()">
                            Odhlásit se
                        </button>
                    </div>
                </div>
                    ...
                    <tr class="${isPickedUp ? 'admin-table__row--picked' : ''}">
                        ...
                        <td class="admin-table__actions">
                            <button class="button button--secondary button--small" 
                                onclick="AdminView.editItem('${item.id}')">✏️</button>
                            <button class="button button--danger button--small" 
                                onclick="AdminView.deleteItem('${item.id}', '${item.name.replace(/'/g, "\\'")}')">🗑️</button>
                        </td>
                    </tr>
                `;
            });
    ...
    editItem(id) {
        ...
        appRoot.innerHTML = `
            <section class="section">
                <div class="page-header">
                    <a href="#/admin" class="back-link">← Zpět na panel</a>
                    <h1 class="section__title">✏️ Upravit: ${item.name}</h1>
                </div>
                <form class="form" onsubmit="AdminView.saveEdit(event, '${item.id}')">
                    <div class="form__group">
